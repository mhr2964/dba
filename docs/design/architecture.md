...
## Invariants (target — not yet fully enforced)

- **`services/` never imports `discord`.** As of 2026-07-22, 11 of the ~84 service files still do (`batch_sim_runner.py`, `cpu_trade_proposals.py`, `cpu_trade_service.py`, `draft_service.py`, `fa_service.py`, `feedback_log.py`, `import_service.py`, `league_service.py`, `news_router.py`, `notifier_service.py`, `playoff_service.py`). Business logic should return plain payload dataclasses; posting to Discord is a presentation concern. `grep -rn "^import discord\|^from discord" services/` is the check.
- **`data/repositories/` owns all SQL.** Raw `pool.fetch*/execute` calls scattered through `services/` should move into a repository module as each is touched. `grep -rn "pool\.\(fetch\|fetchval\|fetchrow\|execute\)" services/` is the check.
- **`Announcer` protocol** (`services/announcer_protocol.py`, once added) is the seam between business logic and Discord posting: `post_embed(channel_key, embed_data)`, `post_text(channel_key, content)`. Each extraction that currently posts directly should implement or consume this instead of importing `discord` itself.
