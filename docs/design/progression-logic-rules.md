...
**Current implementation status (as of 2026-07-25):** P1-P6 shipped as part of a realism audit sweep covering free agency, draft, and progression/aging (companion to `trade-logic-rules.md`'s original trade/scheme audit). The larger version of P6 (role-fit-quality affecting development magnitude) has since shipped in a scoped form as part of realism sweep 3 — see P6 below.
...
**Status:** SHIPPED (extension + threshold fix). Larger version (role-fit compounding signal) SHIPPED in scoped form 2026-07-25 as part of realism sweep 3 — see below.
...
**Larger version — role-fit compounding signal (SHIPPED 2026-07-25, scoped):**

Originally deferred as requiring new cross-service coupling and its own calibration pass with real evidence (same reasoning the original trade audit used to defer evidence-gated items — see `trade-logic-rules.md` B2). Realism sweep 3 shipped a scoped version instead of another blanket defer, once it turned out the plumbing itself was cheap:

- `services/trade_signals/role_fit.py`'s inline threshold logic (comparing a player's tendency fields against `_ROLE_EXPECTED_TENDENCIES`, ±10 tolerance) was extracted into a standalone pure function, `classify_role_fit(tendencies: dict, role: str) -> Literal["mismatch", "match"] | None` — the single place that table and its threshold math now live. `trade_signals/role_fit.py::detect()` (the CPU trade evaluator's signal) calls this same function; behavior there is unchanged, only the implementation moved.
- `data/repositories/player_repo.py`'s `Player` dataclass gained `defense_tendency` (a real column from migration 029 that was already returned by every `SELECT *` query here but silently dropped until now) — needed because `wing_stopper`/`on_ball_pest`/`two_way_wing` in `_ROLE_EXPECTED_TENDENCIES` key off it.
- `run_progression` adds one batched query per call — `SELECT player_id, role FROM player_roles WHERE league_id = $1 AND season = $2` — matching the existing `players` query's batch shape (never per-player). `season` here is the same `pending_progression_season`-derived value P1 already threads through this call, and player_roles rows are derived/persisted under that same season number, so no extra translation is needed; a dedicated test (`test_role_fit_query_aligns_with_season_param`) seeds a player_roles row under the WRONG season and confirms the signal does NOT fire, rather than just assuming the alignment holds — the same discipline P1's own regression test was held to for this exact pipeline.
- `_process_player` looks up the player's role from the batched map and calls `classify_role_fit`, folding the result into `_build_deltas_growth`/`_stable`/`_decline` as a signal that COMPOUNDS with (never replaces) `low_minutes`: `"mismatch"` nudges the same direction `low_minutes` already nudges (a flat `-1` on the `overall` delta only — worse for growth/stable/decline alike), `"match"` nudges the opposite direction (`+1`), and `None` (uncovered role, or no `player_roles` row yet — e.g. a brand-new rookie) applies no adjustment at all, falling back to today's low-minutes-only behavior byte-for-byte.
- **Magnitude is a deliberately conservative, explicitly-labeled placeholder, not a tuned constant.** The trade-eval version of this signal operates on a `±0.15` multiplicative value-scale; progression operates on small integer attribute deltas, and there's no principled way to derive one from the other. The flat `±1` nudge on `overall` only (not a full re-shape of every sub-attribute distribution) is small enough that a wrong initial guess doesn't distort the sim — treat the magnitude itself as unresolved pending real evidence/playtesting.
- **Known, unaddressed coverage gap:** `_ROLE_EXPECTED_TENDENCIES` covers 18 of the 24 roles in `role_scoring.py::ROLE_REGISTRY` — missing `secondary_creator`, `cutter_finisher`, `switching_big`, `glue_guy`, `veteran_mentor`, `developmental`, `end_of_bench`. Every one of those falls back to `None` (no adjustment), which is exactly the documented behavior for an uncovered role, not a bug — but it does mean the signal is silent for a large share of the deep-bench population the base `low_minutes`/`barely_played` penalty already targets. Expanding coverage is a separate, evidence-gated pass, not part of this fix.
- Tests: `tests/test_progression.py::test_role_fit_mismatch_and_match_nudge_growth_overall` / `_stable_overall` / `_decline_overall` (pure, seeded, exact ±1 pairwise comparison against a role_fit=None baseline, for both `low_minutes` states), `test_role_fit_none_is_exact_pre_p6_fallback` (byte-for-byte regression proof against the pre-fix deltas); `tests/test_progression_integration.py::test_role_fit_mismatch_tags_progression_log` / `test_role_fit_match_tags_progression_log`, `test_role_fit_query_aligns_with_season_param`, `test_role_fit_uncovered_role_no_adjustment`, `test_role_fit_missing_player_roles_row_no_adjustment`.
