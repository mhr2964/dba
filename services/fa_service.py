...
async def submit_cpu_offers(league_id: int, season: int) -> int:
    ...
    for team in cpu_teams:
        ...
        # Contenders focus on high-OVR players; rebuilders accept anyone.
        min_ovr = _CPU_CONTENDER_MIN_OVR if cpu_mode == "contending" else 0

        # Years: rebuilding teams prefer short deals (1yr), contenders prefer long (3yr).
        ...
        for fa in free_agents:
            ...
            offer_salary = int(floor * random.uniform(1.0, 1.10))
            offer_salary = max(offer_salary, _MIN_SALARY)

            if cap_used + offer_salary > salary_cap:
                continue

            years = random.randint(1, preferred_years)
...
async def respond_to_counter(
    ...
    if accept:
        ...
        await fa_repo.update_counter_response(pool, counter_id, "accepted")
        await _sign_player(
            pool,
            league_id,
            offer_row["season"],
