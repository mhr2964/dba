...
Full re-architecture sweep in progress (4-phase plan). Phase 0, Phase 1, and the `cpu_trade_proposals.py` half of Phase 2 are complete. `batch_sim_runner.py` is partially split (`sim_persistence.py` done); the rest is paused pending a decision on the untested discord-mixed functions — see Traps.
...
Decide how to handle the rest of `services/batch_sim_runner.py` (3,385 LOC remaining after `sim_persistence.py` was extracted). Three groups of functions are left, in ascending risk order:
1. `_persist_game_result` / `_persist_injuries` and `_maybe_run_cpu_trades` / `_run_cpu_trades_inner` (the plan's remaining `sim_persistence.py` + `cpu_trade_round_trigger.py` targets) — these interleave real DB work with inline `discord.Embed` posting and have **zero test coverage**. A byte-slice relocation (same technique used so far) is possible but would leave `discord` imported in files the plan doesn't intend to allow it in; a full Announcer-protocol conversion needs tests written first since nothing pins their current behavior.
2. The dozen `_maybe_post_*` columnist/report functions (target: `sim_content_pipeline.py` + `sim_channel_announcer.py`) — same untested-discord-mixing problem, larger scale (one function alone is ~825 LOC).
3. `sim_until_rival`/`sim_range`/`sim_single_matchup` (target: slimmed `sim_orchestrator.py`) — depends on 1 and 2 being resolved first.
...
- 2026-07-22 Phase 2, part 2 — extracted `services/sim_persistence.py` (287 LOC) from `batch_sim_runner.py`: `_get_team_roles_cached`, `invalidate_role_cache`, `_stamp_role_data`, `_ensure_lineup`, `_apply_directives`, `_apply_cpu_directives`, `_load_lineup_for_team`, `_team_to_sim_dict`, `_compute_team_ovr` — confirmed zero-`discord` before moving. Updated 3 external callers of `invalidate_role_cache` (`trade_service.py`, `cpu_trade_proposal_runner.py`, `bot/cogs/coach_cog.py`) and one test import. `batch_sim_runner.py` is now 3,385 LOC. Full suite unchanged (242/10/1). Paused here rather than continuing into `_persist_game_result`/`_persist_injuries`/`_run_cpu_trades_inner` — discovered these interleave untested discord-posting with DB work, a materially different risk tier than what was scoped; see Next action.
