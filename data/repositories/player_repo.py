...
class Player:
    ...
    ast_tendency: int = 50
    reb_tendency: int = 50
    # Per-player defensive rating derived from actual stl+blk vs. position
    # average (migration 029). Real DB column, already returned by every
    # `SELECT *` query here but was silently dropped until this mapping.
...
def _player_from_record(r: asyncpg.Record) -> Player:
    ...
    return Player(
        ...
        defense_tendency=r["defense_tendency"] if "defense_tendency" in keys else 50,
