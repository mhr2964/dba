"""CPU-side trade evaluation: live posture mode, the accept/counter/reject
decision (via cpu_trade_acceptance.cpu_should_accept), and counter-offer
construction when the CPU declines but is close enough to bridge the gap.

Extracted from trade_service.py (Phase 3 opportunistic split, see
HANDOFF.md). Only called internally by trade_service.propose -- no
external caller touches this module directly. create_trade_record is
imported locally inside _attempt_counter_offer (deferred) since
trade_service.py imports _cpu_evaluate from here, and importing
create_trade_record at module level would be a circular import.
"""
...
async def _attempt_counter_offer(
    ...
    try:
        ...
        # Deferred import: create_trade_record lives in trade_service.py, which
        # imports _cpu_evaluate from this module -- importing at module level
        # here would be a circular import.
        from services.trade_service import create_trade_record
